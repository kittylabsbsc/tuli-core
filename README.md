This repository contains the core contracts that compose the Tuli Media Protocol.
...
[Media Metadata Schemas Repository](https://github.com/ourTuli/media-metadata-schemas) to allow anyone to support custom metadata.
The only **required** key of the JSON metadata is `version`, which is a string in the format of `<name-calVersion>` (e.g. `Tuli-20210101`).
